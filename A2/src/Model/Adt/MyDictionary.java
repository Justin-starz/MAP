...
public class MyDictionary<T,K> implements MyIDictionary<T, K> {
    ...
    public String toString() {
        ...
        for (Map.Entry<T, K> entry : this.dictionary.entrySet()){
            result.append("Variable name: ").append(entry.getKey()).append(" ").append(entry.getValue()).append(", ");
        }
        if (result.toString().endsWith(", ")) result.delete(result.length()- 2, result.length());
        result.append("]");
